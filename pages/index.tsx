...
interface ConfirmState { type: 'transaction'|'card'|'goal'|'contribution'; id: number; message: string; extra?: number }
...
const EMPTY_FORM    = { type: 'expense', description: '', amount: '', category: 'Outros', date: today(), paymentMethod: 'pix', cardId: '' }
    ...
    <div style={{ position:'fixed', inset:0, background:'#000000D0', display:'flex', alignItems:'center', justifyContent:'center', zIndex:400, padding:20 }}>
      <div style={{ background:'#0A1929', border:'1px solid #EF444440', borderRadius:20, padding:28, maxWidth:340, width:'100%', textAlign:'center' }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button onClick={onCancel} style={{ flex:1, padding:12, background:'#07111F', border:'1px solid #1E3A5F', borderRadius:10, color:'#64748B', fontWeight:600, cursor:'pointer', fontFamily:'inherit' }}>
            Cancelar
          </button>
          <button onClick={onConfirm} style={{ flex:1, padding:12, background:'linear-gradient(135deg,#EF4444,#DC2626)', border:'none', borderRadius:10, color:'#fff', fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
            Sim, excluir
          </button>
        </div>
      </div>
...
export default function Home() {
  ...
  const refetch = useCallback(async () => {
    const [txs, cds, gls] = await Promise.all([
...
const handleLogout = () => {
  ...
  const monthTx = useMemo(() => transactions.filter(t => { const d = new Date(t.date + 'T12:00:00'); return d.getMonth()===currentMonth && d.getFullYear()===currentYear }), [transactions,currentMonth,currentYear])
  ...
  return (
    <>
      ...
      {/* Saving overlay */}
      {saving && <div style={{ position:'fixed', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg,#10B981,#059669)', zIndex:500, animation:'slide 0.8s ease-in-out infinite alternate' }} />}
      <style>{`@keyframes slide{from{opacity:0.6}to{opacity:1}}`}</style>
      ...
      <div style={{ padding:'20px 16px', maxWidth:860, margin:'0 auto' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:16 }}>
            ...
            {goalsStats.length > 0 && (
              <div style={{ background:'#0A1929', border:'1px solid #0F2744', borderRadius:14, padding:16 }}>
                ...
                {goalsStats.length>3 && <div style={{ fontSize:11, color:'#475569', textAlign:'center', marginTop:10 }}>+{goalsStats.length-3} mais na aba Metas</div>}
              </div>
            )}
            ...
            <div style={{ background:'#0A1929', border:'1px solid #0F2744', borderRadius:14, padding:'18px 14px' }}>
              ...
              <div style={{ display:'flex', gap:16, justifyContent:'center', marginTop:4 }}>
                {[['#10B981','Entradas'],['#EF444480','Saídas']].map(([c,l])=>(
                ...
                {categoryData.length>0 ? (
                  <>
                    <ResponsiveContainer width="100%" height={120}>
                      <PieChart>
                        <Pie data={categoryData} dataKey="value" cx="50%" cy="50%" outerRadius={50} innerRadius={30} paddingAngle={2}>
                          {categoryData.map((_,i)=><Cell key={i} fill={PIE_COLORS[i%PIE_COLORS.length]} />)}
                        </Pie>
                        <Tooltip formatter={(v: number)=>fmt(v)} contentStyle={{ background:'#0F2744', border:'1px solid #1E3A5F', borderRadius:8, color:'#E2E8F0', fontSize:11 }} />
                      </PieChart>
                    </ResponsiveContainer>
                      ...
                      {categoryData.slice(0,4).map((c,i)=>(
                        <div key={c.name} style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                          <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                            ...
                            <span style={{ fontSize:11, color:'#94A3B8', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:70 }}>{c.name}</span>
                          </div>
                    ...
                    {[...monthTx].reverse().slice(0,6).map(t=>(
                      <div key={t.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <div style={{ overflow:'hidden' }}>
                          <div style={{ fontSize:12, fontWeight:600, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:100 }}>{t.description}</div>
                          <div style={{ fontSize:10, color:'#475569' }}>{t.category}</div>
                        </div>
                        <div style={{ fontSize:12, fontWeight:700, color:t.type==='income'?'#10B981':'#EF4444', fontFamily:"'DM Mono',monospace", flexShrink:0, marginLeft:4 }}>
        ...
        {activeTab==='transactions' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <div>
                <div style={{ fontWeight:800, fontSize:16 }}>Lançamentos</div>
                <div style={{ fontSize:12, color:'#475569' }}>{MONTHS[currentMonth]} {currentYear} · {monthTx.length} registros</div>
              </div>
                ...
                {[...monthTx].reverse().map(t => {
                  ...
                  return (
                    <div key={t.id} style={{ background:'#0A1929', border:'1px solid #0F2744', borderRadius:12, padding:'12px 14px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                        <div style={{ background:t.type==='income'?'#10B98125':'#EF444425', borderRadius:8, width:34, height:34, display:'flex', alignItems:'center', justifyContent:'center', fontSize:15, flexShrink:0 }}>
                          {t.type==='income'?'⬆':'⬇'}
                        </div>
                        <div>
                          <div style={{ fontWeight:600, fontSize:13 }}>{t.description}</div>
                          <div style={{ fontSize:10, color:'#475569' }}>
                            {t.category} · {t.date.split('-').reverse().join('/')} · {t.paymentMethod==='credit'?`Cartão${card?` (${card.name})`:''}`:t.paymentMethod}
                          </div>
                        </div>
                      </div>
                      <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                        <div style={{ fontWeight:800, fontSize:14, color:t.type==='income'?'#10B981':'#EF4444', fontFamily:"'DM Mono',monospace" }}>
                          {t.type==='income'?'+':'−'}{fmt(t.amount)}
                        </div>
                        <button onClick={()=>askDelete({ type:'transaction', id:t.id, message:`Excluir "${t.description}" de ${fmt(t.amount)}?` })}
                          style={{ background:'#EF444418', border:'1px solid #EF444430', color:'#EF4444', borderRadius:6, width:26, height:26, cursor:'pointer', fontSize:13, display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
                      </div>
                    </div>
                  )
                })}
        ...
        {activeTab==='cards' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <div>
                <div style={{ fontWeight:800, fontSize:16 }}>Cartões de Crédito</div>
                <div style={{ fontSize:12, color:'#475569' }}>{cards.length} cartão(ões) cadastrado(s)</div>
              </div>
              ...
              {cardSpending.map(card => {
                ...
                return (
                  <div key={card.id} style={{ background:`linear-gradient(145deg,${card.color}22,#0A1929)`, border:`1px solid ${card.color}50`, borderRadius:18, padding:20, position:'relative' }}>
                    <button onClick={()=>askDelete({ type:'card', id:card.id, message:`Excluir o cartão "${card.name}"? As compras vinculadas serão mantidas.` })}
                      style={{ position:'absolute', top:12, right:12, background:'#EF444418', border:'none', color:'#EF4444', borderRadius:6, width:22, height:22, cursor:'pointer', fontSize:11 }}>×</button>
                    ...
                    <div style={{ background:'#0A1929', borderRadius:99, height:6 }}>
                      <div style={{ background:pct>80?'#EF4444':card.color, borderRadius:99, height:6, width:`${pct}%`, transition:'width 0.4s ease' }} />
                    </div>
            ...
            <div style={{ background:'#0A1929', border:'1px solid #0F2744', borderRadius:14, padding:16 }}>
              <div style={{ fontWeight:700, fontSize:12, marginBottom:12, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.5px' }}>Compras no Cartão — {MONTHS[currentMonth]}</div>
              {monthTx.filter(t=>t.paymentMethod==='credit').length===0 ? (
                <div style={{ textAlign:'center', color:'#334155', fontSize:12, padding:20 }}>Nenhuma compra no cartão</div>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                  {monthTx.filter(t=>t.paymentMethod==='credit').reverse().map(t => {
                    const card = cards.find(c=>c.id===t.cardId)
                    return (
                      <div key={t.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'8px 0', borderBottom:'1px solid #0F274450' }}>
                        <div>
                          <div style={{ fontSize:13, fontWeight:600 }}>{t.description}</div>
                          <div style={{ fontSize:10, color:'#475569' }}>{t.category} · {card?card.name:'Cartão'}</div>
                        </div>
                        <div style={{ fontWeight:700, fontSize:13, color:'#EF4444', fontFamily:"'DM Mono',monospace" }}>−{fmt(t.amount)}</div>
                      </div>
                    )
                  })}
                </div>
        ...
        {activeTab==='goals' && (
          <div>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
              <div>
                <div style={{ fontWeight:800, fontSize:16 }}>Metas & Projetos</div>
                <div style={{ fontSize:12, color:'#475569' }}>{goalsStats.filter(g=>g.done).length} concluída(s) · {goalsStats.filter(g=>!g.done).length} em andamento</div>
              </div>
            ...
            {goalsStats.length===0 ? (
              <div style={{ textAlign:'center', color:'#334155', padding:'60px 0' }}>
                ...
                <div style={{ fontWeight:600 }}>Nenhuma meta ainda</div>
                <div style={{ fontSize:13, marginTop:4 }}>Clique em "+ Meta" para criar seu primeiro projeto</div>
              </div>
            ) : (
                      ...
                      <div style={{ padding:18 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:14 }}>
                          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize:11, color:'#475569' }}>
                                {g.done ? <span style={{ color:'#10B981', fontWeight:700 }}>✓ Meta concluída! Parabéns!</span>
                                  : g.days<=0 ? <span style={{ color:'#EF4444' }}>Prazo encerrado</span>
                                  : `${g.days} dias restantes · prazo ${new Date(g.deadline+'T12:00:00').toLocaleDateString('pt-BR')}`}
                              </div>
                            </div>
                          </div>
                          <div style={{ display:'flex', gap:6, flexShrink:0 }}>
                            ...
                            <button onClick={()=>askDelete({ type:'goal', id:g.id, message:`Excluir a meta "${g.name}"? Todos os aportes serão perdidos.` })}
                              style={{ background:'#EF444418', border:'none', color:'#EF4444', borderRadius:6, width:28, height:28, cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
                          </div>
                        </div>
                        ...
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:14 }}>
                          ...
                          {!g.done && <span style={{ fontSize:11, color:'#F59E0B' }}>Guardar {fmt(g.needed)}/mês para chegar lá</span>}
                        </div>

                        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8 }}>
                          {[
                            {label:'Falta',val:g.done?'—':fmt(Math.max(g.target-g.saved,0)),color:'#EF4444'},
                            {label:'Aportes',val:String(g.contributions.length),color:'#94A3B8'},
                            {label:'Aporte/mês',val:g.done?'✓':fmt(g.needed),color:'#F59E0B'},
                          ].map(s=>(
                        ...
                        {g.contributions.length>0 && (
                          <button onClick={()=>setExpandedGoal(isExp?null:g.id)} style={{ background:'none', border:'none', color:'#64748B', cursor:'pointer', fontSize:12, marginTop:12, padding:0, fontFamily:'inherit', display:'flex', alignItems:'center', gap:4 }}>
                            {isExp?'▲ Fechar':'▼ Ver'} aportes ({g.contributions.length})
                          </button>
                        )}
                      </div>
                        ...
                        <div style={{ borderTop:'1px solid #0F2744', padding:'12px 18px', display:'flex', flexDirection:'column', gap:8 }}>
                          {[...g.contributions].reverse().map(c=>(
                            <div key={c.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'6px 0', borderBottom:'1px solid #07111F' }}>
                              <div>
                                <div style={{ fontSize:13, fontWeight:600, color:'#10B981', fontFamily:"'DM Mono',monospace" }}>+{fmt(c.amount)}</div>
                                <div style={{ fontSize:10, color:'#475569' }}>{c.date.split('-').reverse().join('/')} {c.note&&`· ${c.note}`}</div>
                              </div>
                              <button onClick={()=>askDelete({ type:'contribution', id:c.id, message:`Excluir aporte de ${fmt(c.amount)}?`, extra:g.id })}
                                style={{ background:'#EF444418', border:'none', color:'#EF4444', borderRadius:6, width:24, height:24, cursor:'pointer', fontSize:12, display:'flex', alignItems:'center', justifyContent:'center' }}>×</button>
                            </div>
                          ))}
                        </div>
                      )}
        ...
        {activeTab==='report' && (
          <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
            ...
            <div style={{ background:'#0A1929', border:'1px solid #0F2744', borderRadius:14, padding:18 }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12, textAlign:'center' }}>
                {[['Entradas',fmt(totalIncome),'#10B981'],['Saídas',fmt(totalExpense),'#EF4444'],['Saldo',fmt(balance),balance>=0?'#10B981':'#EF4444']].map(([l,v,c])=>(
                  <div key={l} style={{ background:'#07111F', borderRadius:10, padding:'12px 8px' }}>
                    <div style={{ fontSize:10, color:'#475569', marginBottom:6, textTransform:'uppercase' }}>{l}</div>
                    <div style={{ fontSize:14, fontWeight:800, color:c as string, fontFamily:"'DM Mono',monospace" }}>{v}</div>
                  </div>
                ))}
              </div>
              <div style={{ marginTop:14, display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                <div style={{ background:'#07111F', borderRadius:10, padding:'10px 14px', display:'flex', justifyContent:'space-between' }}>
                  <span style={{ fontSize:12, color:'#64748B' }}>Taxa de poupança</span>
                  <span style={{ fontSize:12, fontWeight:700, color:'#F59E0B' }}>{savingsRate}%</span>
                </div>
                <div style={{ background:'#07111F', borderRadius:10, padding:'10px 14px', display:'flex', justifyContent:'space-between' }}>
                  <span style={{ fontSize:12, color:'#64748B' }}>Lançamentos</span>
                  <span style={{ fontSize:12, fontWeight:700 }}>{monthTx.length}</span>
                </div>
              </div>
            </div>
            <div style={{ background:'#0A1929', border:'1px solid #0F2744', borderRadius:14, padding:18 }}>
            ...
            <div style={{ background:'#0A1929', border:'1px solid #0F2744', borderRadius:14, padding:18 }}>
              <div style={{ fontWeight:700, fontSize:12, marginBottom:14, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'0.5px' }}>💳 Por Forma de Pagamento</div>
              {[['credit','Cartão de Crédito','#8B5CF6'],['pix','PIX','#10B981'],['débito','Débito','#3B82F6'],['dinheiro','Dinheiro','#F59E0B']].map(([pm,label,color])=>{
                const total = monthTx.filter(t=>t.paymentMethod===pm&&t.type==='expense').reduce((s,t)=>s+t.amount,0)
                if (!total) return null
                return (
                  <div key={pm} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 0', borderBottom:'1px solid #07111F' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:8 }}><div style={{ width:8, height:8, borderRadius:'50%', background:color as string }} /><span style={{ fontSize:13, color:'#94A3B8' }}>{label}</span></div>
                    <span style={{ fontWeight:700, fontSize:13, fontFamily:"'DM Mono',monospace" }}>{fmt(total)}</span>
                  </div>
                )
              })}
            </div>
      ...
      {showModal && (
        <div style={{ position:'fixed', inset:0, background:'#000000B0', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:200 }} onClick={()=>setShowModal(false)}>
          <div style={{ background:'#0A1929', borderRadius:'20px 20px 0 0', padding:'24px 20px 32px', width:'100%', maxWidth:480, border:'1px solid #0F2744', maxHeight:'90vh', overflowY:'auto' }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800, fontSize:16, marginBottom:18 }}>Novo Lançamento</div>
            <div style={{ display:'flex', background:'#07111F', borderRadius:10, padding:4, marginBottom:18, gap:4 }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <div><label style={LS}>Descrição</label><input type="text" placeholder="Ex: Mercado, Salário..." value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} style={IS} /></div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                <div><label style={LS}>Valor (R$)</label><input type="number" placeholder="0.00" value={form.amount} onChange={e=>setForm(p=>({...p,amount:e.target.value}))} style={IS} /></div>
                <div><label style={LS}>Data</label><input type="date" value={form.date} onChange={e=>setForm(p=>({...p,date:e.target.value}))} style={IS} /></div>
              </div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
                ...
                <div><label style={LS}>Pagamento</label>
                  <select value={form.paymentMethod} onChange={e=>setForm(p=>({...p,paymentMethod:e.target.value,cardId:''}))} style={IS}>
                    <option value="pix">PIX</option><option value="credit">Cartão Crédito</option><option value="débito">Débito</option><option value="dinheiro">Dinheiro</option>
                  </select>
                </div>
              </div>
              {form.paymentMethod==='credit' && (
                <div><label style={LS}>Qual Cartão?</label>
            ...
            <div style={{ display:'flex', gap:10, marginTop:20 }}>
              ...
              <button onClick={addTransaction} style={{ flex:2, padding:13, border:'none', borderRadius:10, color:'#fff', fontWeight:700, cursor:'pointer', fontFamily:'inherit', fontSize:14, background:form.type==='income'?'linear-gradient(135deg,#10B981,#059669)':'linear-gradient(135deg,#EF4444,#DC2626)' }}>Salvar Lançamento</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showGoalModal && (
        <div style={{ position:'fixed', inset:0, background:'#000000B0', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:200 }} onClick={()=>setShowGoalModal(false)}>
          <div style={{ background:'#0A1929', borderRadius:'20px 20px 0 0', padding:'24px 20px 32px', width:'100%', maxWidth:480, border:'1px solid #0F2744', maxHeight:'90vh', overflowY:'auto' }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ fontSize:12, color:'#475569', marginBottom:16 }}>Ex: Compra do carro, viagem, reforma...</div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
              <div><label style={LS}>Nome do Projeto</label><input type="text" placeholder="Ex: Compra do Carro" value={goalForm.name} onChange={e=>setGoalForm(p=>({...p,name:e.target.value}))} style={IS} /></div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
      ...
      {showContribModal!==null && (()=>{
        const goal = goals.find(g=>g.id===showContribModal)
        if (!goal) return null
        ...
        return (
          <div style={{ position:'fixed', inset:0, background:'#000000B0', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:200 }} onClick={()=>setShowContribModal(null)}>
            <div style={{ background:'#0A1929', borderRadius:'20px 20px 0 0', padding:'24px 20px 32px', width:'100%', maxWidth:480, border:`1px solid ${goal.color}40` }} onClick={e=>e.stopPropagation()}>
              ...
              <div style={{ background:'#07111F', borderRadius:99, height:5, margin:'14px 0 4px' }}>
                <div style={{ background:goal.color, borderRadius:99, height:5, width:`${Math.min(goal.target>0?(saved/goal.target)*100:0,100)}%` }} />
              </div>
              <div style={{ fontSize:11, color:'#64748B', marginBottom:18 }}>{fmt(saved)} de {fmt(goal.target)} · {goal.target>0?((saved/goal.target)*100).toFixed(1):0}%</div>
              <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
